...
if ($_SERVER['REQUEST_METHOD'] == 'GET') {
    ...
    if (empty($errors) && !empty($_COOKIE[session_name()]) && session_start() && !empty($_SESSION['login'])) {
        try {
            ...
            $db = new PDO('mysql:host=localhost;dbname=u47524', $user, $pass, array(PDO::ATTR_PERSISTENT => true));
            ...
            $select = $db->prepare("select * FROM team2 WHERE user_login = ?");
    ...
    if (!empty($_COOKIE[session_name()]) && session_start() && !empty($_SESSION['login'])) {
        try {
            ...
            $team = $db->prepare("UPDATE team2 SET team = ? WHERE user_login = ? ");
        ...
        try {
            ...
            $team = $db->prepare("INSERT INTO team2 SET team = ?, user_login = ? ");
